...
class ExtractText(StatelessTransform):
    ...
    def transform(self, X):
        ...
        return list(it)
        '''
        it = ([" ".join(nltk.word_tokenize(datapoint.phrase)), datapoint.sentenceid] for datapoint in X)
        if self.lowercase:
            return [[x[0].lower(),x[1]] for x in it]
        return list(it)
        '''
...
class ClassifierOvOAsFeatures:
    ...
    def fit(self, X, y):
        ...
        #oVo = OneVsOneClassifier(SGDClassifier(), n_jobs=-1)
        #self.classifiers= oVo.fit(X, numpy.array(y));
        return self
    ...
    def transform(self, X, y=None):
        ...
        #xs = self.classifiers.decision_function(X).reshape(-1, 1)
